...
train_file = "/Users/shahriari/Downloads/NewTrain.csv"
...
def Expedia_Competition():
     ...
     print('Start Item-based Collaborative Filterting')
 ...
 #    myarray = np.zeros((np.max(SortedUserSet), np.max(SortedLocationList)))
     ...
     print('Start Item-based rating computation')     
 ...
 #              Same_Location_Users=location_user_dict[cityID]
               ...
               if User_Item_Rating.get(user):
                    if User_Item_Rating[user].get(item):
                         continue
               for EachItem in SortedLocationList:
 #                   dict1=User_Item_Rating[user]
#                    dict2=User_Item_Rating[EachUser]
                    '''Cosine Similarity between item and EachItem'''
                    FirstItem=ItemDictionaryReverse[item]
                    SecondItem=ItemDictionaryReverse[EachItem]
                    v1=myarray[:,FirstItem]
                    v2=myarray[:,SecondItem]
...
#                    weight=np.corrcoef([dict1.get(x, 0) for x in keys],[dict2.get(x, 0) for x in keys])[0, 1]
                    ...
                    if(Item_User_Rating.get(EachItem) is None):
                         continue
                    elif(Item_User_Rating.get(EachItem).get(user) is None):
                         continue
                    elif(math.isnan(weight)):
                         continue
                    else:
                         '''Calculating the rating of an user to an item'''
                         total1=total1+(weight*Item_User_Rating.get(EachItem).get(user))
